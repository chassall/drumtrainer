...
% Load RT info
rtInfo = readtable('rt_vector.csv');
    ...
    for i = 1:length(EEG.event)
        if ismember(EEG.event(i).type,[earlyTriggers onTimeTriggers lateTriggers])
            ...
            elseif ~collapseFB && ~collapseT && trialCount <= size(thisRTInfo,1)
                ...
                % thisRTAdjustCat = num2str(thisRTInfo{trialCount,'rt_adjust_cat'});
...
for p = 1:length(ps)
    ...
    % Load beta values
    betaFolder = [dataFolder '/derivatives/eegbetams/' subName];
    betaFile = [subName '_task-drumtrainer_eegbetams.mat'];
...
%% Make the figure (Incorrect waveformns only)
...
for i = 1:3
    ...
    legend(phs,{'Early-0','Late-0','Early-1','Late-1'},'Box','off','Location','NorthWest');
    xlim([-0.2 0.6]);
    ylim([-1 15]);
    xlabel('Time (s)');
    ylabel('Voltage (\muV)');
...
print(fullfile(outputFolder,'erps_mediansplitonrtadjust_cor.tiff'),'-dtiff','-r600');
